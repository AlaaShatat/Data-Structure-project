...
namespace xml_read
{
    class XML
    {
       ...
       public XML (string path_)
        ...
        //COMPLIXTY O(N) WHERE N IS THE TOTAL NUMBER OF CHARACTERS OF THE FILE
       ...
       public void Parse_XML()
       {
           ...
           using (StreamReader rdr = new StreamReader(_inputStream))
           {
               ...
               while ((currentRead = rdr.Read()) >= 0)
               {
                   ...
                   // Tag Detection  COMPLEXTY O(M) WHERE M IS THE TAG NAME LENGTH
                   ...
                   // tag value COMPLIXTY O(L) WHERE L IS THE CURRENT TAG VALUE LENGTH
                   else if (currentRead != '<' && currentRead != '\r' && currentRead != '\n')
                   {
                    ...
                    // closing tag COMPLEXTY O(M) WHERE M IS THE TAG NAME LENGTH
        ...
        // correct result COMPLEXTY O(N) WHERE N IS THE TOTAL NUMBER OF CHARACTERS OF THE XML FILE
        public string correction()
       {
           ...
           using (StreamReader rdr = new StreamReader(_inputStream))
           {
               ...
               while ((currentRead = rdr.Read()) >= 0)
               {
                   ...
                   if ((currentChar == '<') && (rdr.Peek() == '?'))
                   {
                    ...
                    // Tag Detection COMPLIXTY O(M) WHERE M IS THE TAG NAME LENGTH
                    if (currentRead == '<' && rdr.Peek() != '/')
                   {
                    ...
                    // tag value COMPLIXTY O(L) WHERE L IS THE TAG VALUE LENGTH
                    else if (currentRead != '<' && currentRead != '\r' && currentRead != '\n')
                   {
                       string tag_value = "" + Convert.ToChar(currentRead);
                       while (rdr.Peek() != '<')
                    ...
                    else if (currentRead == '<' && rdr.Peek() == '/')
                   {
                       ...
                       rdr.Read();
                       // error correction COMPLEXTY O(1)
                       if (validationStack.Count == 0) 
                       {
                           errorLine += "Error: Tag not closed at Line " + lineNumber + "Col." + columnNumber + Environment.NewLine;
                       }
                       ...
                       //Console.WriteLine(correctResult);
                       Tag poped_node = validationStack.Pop();
                       //Root Node if empty stack. COMPLEXITY OF STACK O(1)
                       if (validationStack.Count == 0)
                       {
                           this.root_tags.Add(poped_node);
                       }
        ...
        //format COMPLEXITY O(K) WHERE K IS THE TOTAL NUMBER OF TAGS (PARENTS  + CHILDREN)
        ...
        //trim COMPLEXTIY O(N) WHERE N IS THE TOTAL NUMBER OF CHARACTERS OF XML FILE
        public string Trim()
       {
       ...
       public void storeOutput(string filepath_, string sotred) 
       {
        ...
        // compression COMPLEXTIY O(N) WHRER N IS THE TOTAL NUMBER OF CHARACTERS IS XML FILE
       public int IsInTable(string searchList)
        ...
        //COMPLEXTITY O(N) WHERE N IS THE TOTAL NUMBER OF CHARACTERS IN THE XML FILE
        ...
        // print result COMPLEXTY O(K) WHERE K IS THE TOTAL NUMBER OF ROOTS
